...
func (tray *Tray) setup() error {
	...
	tray.SetToolTip(l18n.Sprintf("WGSafe: Deactivated"))
	...
	for _, item := range [...]struct {
		...
		{separator: true},
		{label: l18n.Sprintf("&About WGSafe…"), handler: tray.onAbout, enabled: true},
			...
			if tunnelAction != nil {
				...
				case manager.TunnelStarted:
					if !wasChecked {
						...
						tray.ShowCustom(l18n.Sprintf("WGSafe Activated"), l18n.Sprintf("The %s tunnel has been activated.", tunnel.Name), icon)
					}

				case manager.TunnelStopped:
					if wasChecked {
						icon, _ := loadSystemIcon("imageres", -31, 128) // TODO: this icon isn't very good...
						tray.ShowCustom(l18n.Sprintf("WGSafe Deactivated"), l18n.Sprintf("The %s tunnel has been deactivated.", tunnel.Name), icon)
					}
				}
			}
		} else if !tray.mtw.Visible() {
			tray.ShowError(l18n.Sprintf("WGSafe Tunnel Error"), err.Error())
		}
...
func (tray *Tray) updateGlobalState(globalState manager.TunnelState) {
	...
	tray.SetToolTip(l18n.Sprintf("WGSafe: %s", textForState(globalState, true)))
...
func (tray *Tray) UpdateFound() {
	...
	showUpdateBalloon := func() {
		...
		tray.ShowCustom(l18n.Sprintf("WGSafe Update Available"), l18n.Sprintf("It is time to use Amneziavpn. You are advised to update as soon as possible."), icon)
	}
