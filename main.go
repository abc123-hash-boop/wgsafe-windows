...
func checkForWow64() {
	...
	if b {
                fatalf("You must use the native version of WGSafe on this computer.")
	}
}
...
func checkForAdminGroup() {
	...
	if !elevate.TokenIsElevatedOrElevatable(processToken) {
                fatalf("WGSafe may only be used by users who are a member of the Builtin %s group.", elevate.AdminGroupName())
	}
}
...
func checkForAdminDesktop() {
	...
	if !adminDesktop && err == nil {
                fatalf("WGSafe is running, but the UI is only accessible from desktops of the Builtin %s group.", elevate.AdminGroupName())
	}
}
...
func main() {
	...
	case "/installmanagerservice":
		...
		time.Sleep(30 * time.Second)
                fatalf("WGSafe system tray icon did not appear after 30 seconds.")
		return
